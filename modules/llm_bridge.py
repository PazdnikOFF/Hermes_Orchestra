...
def _build_agent_system(agent: AgentState, extra_context: str = "",
    ...
    if workspace:
        parts.append(
            ...
            "ДОВЕДИ ДО КОНЦА за этот проход. Если в задаче перечислены компоненты "
...
def judge_evaluate(direction: str, task: str, output: str, soul_prompt: str,
                   artifacts: str = "") -> dict:
    """
    ...
    system = (
        ...
        "  3. ПОЛНОТА/ЗАВЕРШЁННОСТЬ. Если задача требует НЕСКОЛЬКО компонентов "
        "(например: парсер, БД, REST API, графики, Telegram-уведомления, Docker/systemd) — "
        "результат обязан содержать ВСЕ как готовые артефакты. Если часть отложена в "
        "«следующие шаги»/«TODO»/«предлагается» или отсутствует среди созданных файлов — "
        "это НЕ выполнено: score ≤ 0.4, в critique перечисли каждый недостающий компонент "
        "и что именно доделать. Сверяй заявленное с фактическим списком файлов (если дан).\n\n"
        "Шкала score:\n"
    ...
    if artifacts:
        content += (
            "\n\nФактически созданные файлы в рабочем каталоге проекта "
            "(сверь полноту с этим списком — недостающие компоненты = неполнота):\n"
            f"{artifacts}"
        )
